...
class LaneModule(pl.LightningModule):
    ...
    def get_dataloader(self, dataset_type):
        if self.dataset == "once":
            ...
            ds = NUScenesDataset(dataset_type=dataset_type, multitask=self.multitask if not self.intervention else "intervention", ground_truth=self.ground_truth, dataset_path=self.dataset_path)
